...
Agent Inovasi is designed to isolate each person's data and activity by scope. It is early,
...
Agent Inovasi's interactive agent surfaces currently assume one organization of authenticated
...
create an Agent Inovasi principal or authorize interaction with the agent or control plane. Agent Inovasi is
...
preserve attribution and audit evidence. Agent Inovasi does not guarantee correct model output
...
- The deployment operator controls the cloud account, network, identity provider,
  ...
  grants. Agent Inovasi does not protect a deployment from a malicious or compromised operator.
...
- A published app and its runtime are a separate trust boundary. App code receives
  ...
  use configured per-app acting-as access. Agent Inovasi keeps ambient author credentials out of
  the app, but does not review app code or guarantee how it handles visitor data.

### What the controls do and do not guarantee

Agent Inovasi resolves a principal and scope for each turn, separates scope workspaces, uses
...
- **Browser actions sit outside some core gates.** Actions inside the browser runner
  ...
  browser provider rather than Agent Inovasi's egress proxy.
