  ...
  const runPrompt = async (turn: HarnessTurnInput, toolsEnabled = true): Promise<HarnessTurnResult> => {
    ...
    const threadStartRequest = {
      ...
      developerInstructions:
        "Use the supplied dynamic Agent Inovasi tools for all workspace, execution, memory, history, and surface operations. The built-in working directory is an empty read-only control jail, not the user's workspace.",
      dynamicTools,
