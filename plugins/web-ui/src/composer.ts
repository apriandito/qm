...
function modelOptionFor(value: ModelOptionValue): ModelOption {
...
function isUnsentNewChat(): boolean {
  return (
    ...
    !(chatState.agent?.state.messages ?? []).some((m) => !(m as { opener?: boolean }).opener)
  );
}

function persistDraft(): void {
  if (!chatState.threadRef) return;
  saveDraft(chatState.threadRef, composerState.draft);
  if (isUnsentNewChat()) saveDraft(newChatDraftKey(appState.me?.user), composerState.draft);
}

function clearActiveDraft(): void {
  if (chatState.threadRef) clearDraft(chatState.threadRef);
  if (chatState.sessionId === null) clearDraft(newChatDraftKey(appState.me?.user));
}
...
export const composerState = {
  ...
  skillsCache: null as SkillItem[] | null,
...
async function changeScopeRuntime(
  ...
  const request = ++runtimeRequest;
  const scopeId = chatState.scopeId;
  try {
...
export function resyncModelSelection(): void {
  ...
  composerState.effortLevel = loadStoredEffort(defaultEffortForModel(currentModelOption().model));
}

export function composerForm(agent: Agent): TemplateResult {
...
function openPasteView(id: string, agent: Agent): void {
  ...
  drawActiveChat(agent);
  requestAnimationFrame(() => chatState.host?.querySelector<HTMLTextAreaElement>(".paste-dialog-text")?.focus());
}
...
function insertPasteIntoDraft(agent: Agent): void {
  const view = composerState.pasteView;
  if (!view) return;
  const ta = chatState.host?.querySelector<HTMLTextAreaElement>(".composer-input");
  ...
  requestAnimationFrame(() => {
    const input = chatState.host?.querySelector<HTMLTextAreaElement>(".composer-input");
    if (!input) return;
    input.focus();
...
function currentSlashMenu(): { open: boolean; loading: boolean; matches: SkillMatch[] } {
  ...
  const matches = composerState.skillsCache ? matchSkills(query, composerState.skillsCache) : [];
...
async function loadSkills(agent: Agent): Promise<void> {
  if (skillsLoading || composerState.skillsCache !== null) return;
  skillsLoading = true;
  drawActiveChat(agent);
  try {
    const r = await api<{ skills: SkillItem[] }>("/api/skills");
    composerState.skillsCache = r.skills ?? [];
  } catch {
...
export function focusComposerEnd(): void {
  requestAnimationFrame(() => {
    const ta = chatState.host?.querySelector<HTMLTextAreaElement>(".composer-input");
    if (!ta) return;
    if (ta.disabled) {
...
function onDraftInput(e: InputEvent, agent: Agent): void {
  ...
  if (armed && composerState.skillsCache === null && !skillsLoading) void loadSkills(agent);
  const popoverShown = Boolean(chatState.host?.querySelector(".slash-popover"));
...
function composerCanSend(): boolean {
  return (
    ...
    activeRuntimeConfig !== null &&
    chatState.resolvingApprovals.size === 0 &&
    !hasUnresolvedApproval()
  );
}

function syncComposerControls(agent: Agent): void {
  if (!chatState.host || agent !== chatState.agent) return;
  const send = chatState.host.querySelector<HTMLButtonElement>(".send-btn");
...
function stopStreaming(agent: Agent): void {
  void signalLiveRun("abort").catch((e) => swallow("web-ui: abort signal", e));
  agent.abort();
}

async function sendSteer(agent: Agent): Promise<void> {
  const text = composerState.draft.trim();
  if (!text || !hasLiveRun()) return;
  if (chatState.threadRef) bumpSessionActivity(chatState.threadRef);
  clearActiveDraft();
  composerState.draft = "";
  composerState.error = "";
...
async function sendPrompt(agent: Agent): Promise<void> {
  ...
  if (hasUnresolvedApproval()) return;
  if (agent.state.isStreaming) return sendSteer(agent);
  const text = composerState.draft.trim();
  if (!text && composerState.attachments.length === 0) return;
  if (chatState.threadRef) {
    bumpSessionActivity(chatState.threadRef);
    chatState.pendingSend = chatState.threadRef;
    renderList();
  }
...
export async function onDrop(e: DragEvent, agent: Agent): Promise<void> {
  if (!dragHasFiles(e)) return;
  e.preventDefault();
  dragDepth = 0;
  composerState.dragging = false;
...
function pickFiles(): void {
  if (hasUnresolvedApproval() || chatState.resolvingApprovals.size > 0 || chatState.agent?.state.isStreaming) return;
  chatState.host?.querySelector<HTMLInputElement>(".file-input")?.click();
}
...
function selectModel(value: string, agent: Agent): void {
  const option = getModelOptions().find((candidate) => candidate.value === value);
  if (!option) return;
  const previousDefaultEffort = defaultEffortForModel(currentModelOption().model);
  if (chatState.threadRef) rememberThreadPick(chatState.threadRef, option.value);
...
function selectHarness(harnessId: string, agent: Agent): void {
  ...
  const options = getModelOptionsForHarness(harnessId);
...
export function resizeComposer(): void {
  requestAnimationFrame(() => {
    const ta = chatState.host?.querySelector<HTMLTextAreaElement>(".composer-input");
    if (!ta) return;
    ...
    ta.style.height = `${Math.min(cap, Math.max(embedMode ? 0 : 48, content))}px`;
    if (content > cap) {
