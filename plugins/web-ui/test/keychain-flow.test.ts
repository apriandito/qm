...
test("destructive controls settle duplicate attempts while a mutation is busy", () => {
  ...
  assert.match(connectorsSource, /connectorNotice = "Masih ada perubahan keychain lain yang lagi jalan\."/);
...
test("keychain rows reserve success badges for actionable states", () => {
  ...
  assert.match(connectorsSource, /expired \? html`<span class="kc-state warning">Kedaluwarsa<\/span>` : ""/);
  assert.match(connectorsSource, /<span class="kc-state warning">Perlu sambung ulang<\/span>/);
});
