...
test("recurring schedule detail shows first run before the first fire", () => {
  ...
  assert.match(detail, /^Setiap 1d - run pertama /);
});
...
test("one-shot schedule detail keeps the configured run time", () => {
  ...
  assert.match(detail, /^Sekali jalan - run /);
});
...
test("calendar schedule without a core-provided next fire degrades to last/never, not a bogus time", () => {
  ...
  assert.equal(cronRunSummary(cron), "belum pernah jalan");
  assert.match(cronRunSummary({ ...cron, lastFiredAt: Date.UTC(2026, 0, 1, 17) }), /^terakhir /);
});
...
test("same instant renders as today's time in one timezone and tomorrow's in another", () => {
  ...
  assert.equal(formatCronDateTime(ms, now, "Asia/Tokyo"), `besok ${zonedTime(ms, "Asia/Tokyo")}`);
});
...
test("tomorrow classification survives the DST spring-forward boundary", () => {
  ...
  assert.equal(
    ...
    `besok ${zonedTime(tomorrowMs, "America/Los_Angeles")}`,
  );
  assert.doesNotMatch(formatCronDateTime(dayAfterMs, now, "America/Los_Angeles"), /^besok /);
});
...
function legacyFormatCronDateTime(ms: number, now: number, timeZone?: string): string {
  ...
  if (timeZone) {
    ...
    if (parts && todayParts) {
      ...
      if (legacySameLocalDay(parts, tomorrow)) return `besok ${time}`;
