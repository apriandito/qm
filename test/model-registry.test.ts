...
test("exposure is provider-key-aware: a model whose provider is unconfigured is not serviceable", () => {
  const noOpenai = { anthropic: true, openai: false, openrouter: false, deepseek: false };
...
test("provider-key gating applies only to key-authed harnesses (no over-hiding on CLI-auth harnesses)", () => {
  const noKeys = { anthropic: false, openai: false, openrouter: false, deepseek: false };
  ...
  assert.deepEqual(
    modelProviderAvailabilityFor("pi", noKeys, { anthropic: false, openai: true, openrouter: true, deepseek: false }),
    {
      anthropic: false,
      openai: true,
      openrouter: true,
  ...
  assert.deepEqual(
    modelProviderAvailabilityFor("codex", { anthropic: false, openai: true, openrouter: false, deepseek: false }),
    {
      anthropic: false,
      openai: true,
      openrouter: false,
